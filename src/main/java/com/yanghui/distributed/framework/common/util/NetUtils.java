...
package com.yanghui.distributed.framework.common.util;
...
public class NetUtils {
    ...
    public static int getAvailablePort(String host, int port, int maxPort) {
        if (isAnyHost(host)
            ...
            for (int i = port; i <= maxPort; i++) {
                ...
                try {
                    ...
                    if (log.isDebugEnabled()) {
                        log.debug("ip:{} port:{} is available", host, i);
                    }
                    return i;
                } catch (IOException e) {
                    // continue
                    if (log.isWarnEnabled()) {
                        log.warn("Can't bind to address [{}:{}], " +
            ...
            throw new RpcRuntimeException("Can't bind to ANY port of " + host + ", please check config");
        } else {
            throw new RpcRuntimeException("The host " + host
    ...
    public static InetAddress getLocalAddress() {
        ...
        try {
            ...
            if (log.isWarnEnabled()) {
                log.warn("Error when retrieving ip address: " + e.getMessage(), e);
            }
        }
        try {
    ...
    private static InetAddress getLocalHostBySocket(InetSocketAddress remoteAddress) {
        ...
        try {
            ...
            log.warn("Can not connect to host {}, cause by :{}",
        ...
        if (StringUtils.isNotBlank(whiteList)) {
            ...
            for (String ips : whiteList.replace(',', ';').split(";", -1)) {
                try {
                    ...
                    log.warn("syntax of pattern {} is invalid", ips);
                }
            }
        }
        return false;
